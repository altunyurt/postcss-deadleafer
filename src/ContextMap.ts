...
import { DisplayValue, PositionValue, FloatValue, ElementContext } from './types';
...
export class ContextMap {
  ...
  private readonly blockifyingProperties = new Set<string>([
    'position: absolute',
    'position: fixed',
    'float: left',
    'float: right'
  ...
  updateContext(property: string, value: string): void {
    switch (property) {
      case 'display':
        this.context.display = value as DisplayValue;
        this.context.isBlockified = this.checkBlockification();
        break;
      case 'position':
        this.context.position = value as PositionValue;
        this.context.isBlockified = this.checkBlockification();
        break;
      case 'float':
        this.context.float = value as FloatValue;
        this.context.isBlockified = this.checkBlockification();
        break;
    }
  }

  private checkBlockification(): boolean {
    // Blockification occurs when:
    // 1. position is absolute or fixed
    // 2. float is left or right
    return (
      this.context.position === 'absolute' ||
  ...
  isPropertyAllowed(property: string): { allowed: boolean; reason?: string } {
  ...
  private getDisplayGroup(): Set<string> {
  ...
  private generateDisallowedReason(property: string): string {
    const display = this.context.display;
