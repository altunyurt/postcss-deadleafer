...
class PropertyValidator {
    ...
    validate(property, value, selector) {
    ...
    validateSpecificRules(property, value) {
        const context = this.contextMap.getCurrentContext();
        const display = context.display;
        // Check conflict matrix
        if (this.conflictMatrix[property]) {
            const invalidContexts = this.conflictMatrix[property];
    ...
    createError(property, value, reason, selector, line, column) {
        return {
            property,
            value,
            reason,
